...
public class ClusterDataDriver extends BaseGeoDataDriver {
	...
	public List getData() {
		...
		List<DeviceBean> artifactsList = (List) RestTrackingJob.getInstance().getRestServiceList(IEPICConstants.KEY_VEHICLE) ;//Cache.retrieve(datasource.getCacheKey());
		int count = artifactsList == null ? 0 : artifactsList.size();
		DeviceBean geoArtifact = null;

		DblClusters<List<DeviceBean>> clusters = new DblClusters<List<DeviceBean>>(2, datasource.getMaxClusters() == 0 ? 10 : datasource
				.getMaxClusters());
		...
		for (int i = 0; i < count; i++) {
			...
			coord = new Coordinate(StringUtils.getDouble(geoArtifact.getLongitude()), StringUtils.getDouble(geoArtifact.getLatitude()));//Coordinates()[0];
			if (camera.contains(coord)) {
				xs[0] = coord.x;
				xs[1] = coord.y;
				m = Math.random(); //StringUtils.getDouble((String) geoArtifact.getAttribute("time"));
