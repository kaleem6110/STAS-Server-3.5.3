...
public class RBRegionsUtils implements IEPICConstants {
	...
	public static synchronized String[] getZoneNames(GeoArtifact artifact,String layerName, long layerId, String contains, String avoidRegionStartsWith){
		...
		//retrieve the go-fence location details
		UserPreferences rbregions =  new UserPreferences();
		rbregions.setPreferenceTypeId((long)UserPreferenecsHelper.PREFERENCE_ID_RBREGIONS);
		
		List<UserPreferences> rubberbandregionsList = rbregions.searchData();
		...
		if(rubberbandregionsList != null){
			...
			for(int i=0; i<rubberbandregionsList.size(); i++){
				UserPreferences userpreference = rubberbandregionsList.get(i);
				Rubberbandregions rbRegions = (Rubberbandregions) CastorUtils.unmarshalString(userpreference.getPreferenceValue(), Rubberbandregions.class);
				if(rbRegions != null  ){
	...
	private static List<UserPreferences> getUserModulePreferences(long domainId, long languageId, long userId, String userUniqueIdStr, long moduleId) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (domainId == userPrefrences.getDomainId() && languageId == userPrefrences.getLanguageId()
					&& (userUniqueIdStr.equals(userPrefrences.getUserUniqueId()))
					&& moduleId == userPrefrences.getModuleId()) {
				userPreferenceList.add(userPrefrences);
			}
			
			if (domainId == userPrefrences.getDomainId() && languageId == userPrefrences.getLanguageId()
					&& ( StringUtils.isNull(userPrefrences.getUserUniqueId())  )
					&& moduleId == userPrefrences.getModuleId()) {
				userPreferenceList.add(userPrefrences);
			}
		}
	...
	/**
	 * Returns the list of  module preferences
	 * @param domainId
	 * @param languageId
	 * @param userId
	 * @param userUniqueIdStr
	 ...
	 * @return
	 */
	public static UserPreferences[] getUserModulePreferences(long domainId, long languageId, long userId,String userUniqueIdStr, long moduleId, int preferenceId) {
		...
		for (int i = 0; i < userPreferences.length; i++) {
			if(userPreferences[i] != null){
				...
				for (int j = 0; j < rubberbandregionsTemp.getRubberbandregionCount(); j++) {
					if(StringUtils.isNull(regionPrefix ) || rubberbandregionsTemp.getRubberbandregion(j).getRegionName().toLowerCase().contains(regionPrefix.toLowerCase()) ){
