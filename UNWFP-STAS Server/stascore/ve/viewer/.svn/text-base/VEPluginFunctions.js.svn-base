...
/*    
    function GetMap(){
        ...
        map.AttachEvent("onerror", onMapError);
        map.AttachEvent("onresize", onMapResize);
...
*/    
    function GetMap(mapMode){
    	if(parent.window && parent.window.virtualEarthMapMode){
    		mapMode = parent.window.virtualEarthMapMode;
    	}
        map = new VEMap('map3d');
        if(mapMode==null || mapMode=='3D' || mapMode=='null'){
            map.LoadMap(new VELatLong(40.3130432088809,-96.416015625), 3, 'a', false, VEMapMode.Mode3D, true);
        }else{
            map.LoadMap(new VELatLong(40.3130432088809,-96.416015625), 3, 'a', false, VEMapMode.Mode2D, true);
        }
        map.AttachEvent("onchangeview", viewChanged);
        //map.AttachEvent("onclick", shapeInfo);
        //map.AttachEvent("onmouseup", globeWindowEventListener);
    ...
    function mapResize(){ 
       /* 
        ...
        oldAltitude = map.GetAltitude();
        oldHeading = map.GetHeading();
	...
	mapDiv.style.height = myHeight;
	mapDiv.style.width = myWidth;
        map.Resize();
        
        
       /*
        map.Resize(myWidth, myHeight);
        map.SetAltitude(oldAltitude);
        map.SetHeading(oldHeading);
        map.SetPitch(oldPitch);
        map.PanToLatLong(oldMapCenter);
        map.SetCenterAndZoom(oldMapCenter,oldZoom);
       */
    }
    
    function onMapError(mapEvent){
    	alert(mapEvent.error);
    }
    
    function onMapResize(mapEvent){
    	refreshAllControls();
    }
    ...
    function viewChanged(mapEvent){
        ...
        setTimeout("parent.refreshWindows();",2000);
    }
    ...
    function addLayer(name,description){
        ...
        layer1.SetTitle(name);
        layer1.SetDescription(description);
        layersArr[name] = layer1;
        map.AddShapeLayer(layer1);
    	
    	layerNamesArr.push(name);
        /*
        //Works only in 2D...
    ...
    function getFlashMovieObject(movieName){        
        ...
        if (navigator.appName.indexOf("Microsoft Internet")==-1){
          if (document.embeds && document.embeds[movieName])
            ...
            return document.parentWindow.parent.getElementById(movieName);
        }
    }

    function sendToFlash(method,data) {
        var flashMovie=getFlashMovieObject("geFrame");
...
*/
    ...
    /*
     * Called from the Controller to add a new image based screen overlay on the map
     */
    function addOverlay(overlayName,top,left,topUnit,leftUnit,iconPath,description,layerName){
        ...
        if(overLays==null){
            screenOverlaysArr[layerName] = [{name:overlayName,top:top,left:left,topUnit:topUnit,leftUnit:leftUnit,img:imgObj}];
        }else{
    ...
    function deleteControl(controlName){
        ...
        if (myControl!=null){
            ...
            //document.removeChild(myControl);
        }else{
            return;
        }
    ...
    function refreshAllControls(){
    	for(i=0;i<layerNamesArr.length;i++){
	    var overLays = screenOverlaysArr[layerNamesArr[i]];
	    if(overLays!=null){
	        for(j=0;j<overLays.length;j++){
	    	    //repositionControl(overLays[j].name,layerNamesArr[i]);
	    	    deleteControl(overLays[j].name);
	    	    setTimeout("addControl2('"+overLays[j].name+"','"+layerNamesArr[i]+"')", 2000);
	        }
	    }
    	}
    ...
    /*
    function repositionControl(controlName,layerName){
        var myControl = document.getElementById(controlName);
        if (myControl==null){
            return;
        }
        ...
        if(overlay.topUnit=="fraction"){
            top = mapDiv.offsetHeight*(1-overlay.top);
        }   
        if(overlay.leftUnit=="fraction"){
            left = mapDiv.offsetWidth*overlay.left;
        }
        
        var el = overlay.img;
        w = el.width;
        h = el.height;
        myControl.style.top = top-(h/2);
        myControl.style.left = left-(w/2);
        
        var myShim = document.getElementById(controlName+"shim");
        myShim.style.top  = myControl.offsetTop;
        myShim.style.left = myControl.offsetLeft;
    }
    */
    ...
    function addControl2(controlName,layerName){
        ...
        el.id = controlName;
        el.style.top = top-(h/2);
        el.style.left = left-(w/2);
