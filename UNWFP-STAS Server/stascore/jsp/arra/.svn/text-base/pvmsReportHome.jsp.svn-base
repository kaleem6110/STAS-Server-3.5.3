...
<head>
	...
	document.forms[0].xlsExport.disabled=false;
		...
		function showReport() {
			...
			f.src = '<%=ServerUtils.getContextName()%>/projectManagementReportServlet?rptName=pvmsReport&cycledate='+cycName + "&randValue=" + Math.random();
			f.style.display="";
		}
		function exportToExcel() {
			cycName = document.forms[0].cycleNameId.value;
			var f = document.getElementById('iframe1');
			f.src = '<%=ServerUtils.getContextName()%>/projectManagementReportServlet?rptName=pvmsReport&type=xls&cycledate='+cycName + "&randValue=" + Math.random();
			f.style.display="";
		}
	</script>
</head>
