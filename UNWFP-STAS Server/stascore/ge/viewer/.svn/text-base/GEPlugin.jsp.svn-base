...
<script type="text/javascript" src="<%=serverUrl %>js/ge/geplugin.js"></script>
<script type="text/javascript" src="<%=serverUrl %>js/ge/geplugin-helpers.js"></script>
<script type="text/javascript" src="<%=serverUrl %>js/ge/kmldomwalk.js"></script>
<script type="text/javascript" src="<%=serverUrl %>js/ge/math3d.js"></script>
...
%>
<script type="text/javascript" src="<%=serverUrl+pluginJSUrl.trim() %>" ></script>
<%
    }
%>
<script>
    ...
    var locale = "<%=languageCd%>".replace("_","-");
    var GEE_SERVER_URL = "<%=geeServerUrl%>";
    var GEE_SERVER_USERID = "<%=geeServerUserId%>";
    var GEE_SERVER_PASSWORD = "<%=geeServerPassword%>";
  ...
  %>
    google.load("earth", "1", {"other_params":"client=<%=geClientId%>"});
    google.load("maps", "2", {"other_params":"client=<%=geClientId%>"});
  <%
    }
  %>
...
%>
        defaultStaticKmlUrlList.push("<%=kmlUrl%>");
<%
        }
    }
%>
...
function _startAlertTour(){
     ...
     * var alertKmlUrl = serverUrl + "XMLDataRenderer?xmldatatype=com.enterprisehorizons.magma.renderer.component.AlertTourKMLRenderer&groupId=<%=groupId%>&domainId=<%=domainId%>&moduleId=<%=moduleId%>&rand="+Math.random();
...
function _setHighlightPlacemarkUrl(url) {
 ...
 function removeAllChildren(featuresArray) {
 ...
 function _highlight(coordStr) {
        ...
        removeAllChildren(highlightPlacemarksArray);
        highlightPlacemarksArray = [];
            //placemark = null;
        //}
      if(coordStr == null || coordStr == '') {
          return;
      }
     ...
     // if(placemark == null) {
      ...
      //} else {
          //placemark.getStyleSelector().getIconStyle().getIcon().setHref(highlightPlacemarkUrl);
          //point = placemark.getGeometry();
      //}
          ...
          highlightPlacemarksArray[i] = placemark;
      }
 }
...
/* Ruler code - start */
    ...
    function distnaceCalculation(mode){
        ...
        if(mode == 'active'){
            ...
            for(remCntr=0;remCntr<gblPlacemarkArr.length;remCntr++){
                removeAllChildren(gblPlacemarkArr[remCntr]);
                removeAllChildren(gblLineArr[remCntr]);
            }
    ...
    function geDistanceClickListener(kmlEvent){
        try{
            ...
            endPlacemark.setName(distance.toFixed(2) +" mi");
...
*/
    ...
    function getDistance(startPlacemark , endPlacemark ){
        if(startPlacemark != null && endPlacemark != null){
            ...
            return (new geo.Point(startPlacemark.getGeometry()).distance(new geo.Point(endPlacemark.getGeometry()))/ 1609.344).toFixed(2);
        }
    }
    ...
    function getIcnoStyle(){
        ...
        imgUrl = '<%=serverUrl %>' + "images/dot.png";
	...
	*/
    ...
    function getListener(listenerType) {
        if(listenerType == LISTENER_TYPE_MARKER_PLACEMARK) {
            ...
            return globeWindowEventListener;
        }
    }
    /**
...
//********* Smart Art code   *********//
    ...
    function loadSmartArt(layerName){
        ...
        for(idx=0;idx<smartArtDataArr.length;idx++){
            if(smartArtDataArr[idx].name==layerName){
                var baseUrl = serverUrl+"smartart?package="+smartArtDataArr[idx].url+
    ...
    */
    ...
    function smartArtHighlight(mouseEvent){
        var magmaVSUrl = "<%=serverUrl %>PNGStreamer";
        ...
        for(i=0;i<smartDrawOverlayArr.length;i++){
            ...
            if(x >= newX1 && x <= newX2 &&
                y >= newY1 && y <= newY2 ){
                var glowUrl = serverUrl+"smartart?package="+smartArtData.url+"&datatype=highlight&resource="+smartDrawOverlayArr[i].highlightImage;
                var randomnumber=Math.floor(Math.random()*1001);
...
<body  onload='setLocale();init();' scroll=no>
<!--
