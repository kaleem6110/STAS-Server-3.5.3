...
<HTML>
  ...
  <FORM NAME="viewTableForm" ACTION="processTableData.jsp" METHOD="POST">
    ...
    <TABLE>
    	<% for(int i=0;i<fields.length;i++){%>
	  		<TR>
	  			...
	  			<%if(eMgr.getIdName()!=null && eMgr.getIdName().equals(fields[i])){%>
		  			<TD><input type="text" name="txtField<%=i%>" id="txtField<%=i%>" READONLY="true"></TD>
		  			<TD></TD>
	  			<%}else{%>
	  		    	<TD><input type="text" name="txtField<%=i%>" id="txtField<%=i%>"></TD>
		...
		<TR><TD CLASS="dataCell" COLSPAN="<%=fields.length%>"> <%=modelName%> </TD></TR>    	
		<TR>
	...
	%>
     	<TR>
     	    <TD CLASS="dataHeaderCell" COLSPAN="<%=fields.length%>">
     	    	...
     	    	<INPUT TYPE="text"   ID="pageNo"  size="5" NAME="pageNo" value="<%=pageNo%>" onkeypress="onEnterGoToPage();" >&nbsp;
     	    	of <%=totalNoOfPages%> &nbsp;
