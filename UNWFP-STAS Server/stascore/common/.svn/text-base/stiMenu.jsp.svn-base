...
<% 
int i=0;
 request.setAttribute("rasEnabled",""+ProcessManagerHelper.isRasLinkEnabled());
%> 
<% try { %>
		 ...
		 <logic:notEmpty name="stiModulesList" property="url">
			<logic:match name="stiModulesList" property="url" value="myaction">
				 <logic:equal name="rasEnabled" value="true">
						<li id='<%=i %>' >
							<span id="<bean:write name="stiModulesList" property="name" />" onmouseover="changeStyle('<%=i %>')" onmouseout="resetStyle('<%=i %>')"  onclick="return loadToIFrameUrl('<bean:write name="stiModulesList" property="url"  />', '<%=i %>')" >
