...
function openInfoframe(idvalue, rowObject, clickedRadioId){
    ...
    try{
          ...
          infoFrameWin = new Window({id: "infoFrameWin", className: "alphacube", width:(screen.width)/(3/2), height:(screen.height)/2,top:100,left:100, title:" <bean:write property="displayModelName" name="masterTableViewForm" />", closable:true, wiredDrag: true, url: urlToOpen}); 
...
<script>
           ...
           function operator(display,isString,isNum,isDate,isBln,value) {
              ...
              function addSearchRow() {
                        ...
                        var strHtml = "<select name='"+fieldId+"'  id='"+fieldId+"'  dojoType='dijit.form.FilteringSelect' trim='value' class='selectbox'  store='fieldStore' searchAttr='label' onchange='loadOperator(this.id,this.value)'> <option value=''></option> </select>";
                        
                        td1.innerHTML = strHtml;

                        var strHtml1 = "<select name='"+operatorId+"'  id='"+operatorId+"'   trim='value' dojoType='dijit.form.FilteringSelect' class='selectbox' store='operatorStore'  searchAttr='label' onchange='displayField(this.id,this.value)'> <option value=''></option> </select>";
                                                
                        td2.innerHTML = strHtml1;

                        var userInput="<div id='searchDataDiv"+count+"' ><input type='text' id='"+searchDataId+"' name='"+searchDataId+"'  dojoType='dijit.form.ValidationTextBox'  trim='true' ucfirst='true' /></div>";
                        userInput=userInput+"<div id='searchDataDateDiv"+count+"' style='display:none' > <input id='searchDataDate"+count+"' type='text' style='width:194px;height:1.7em;' dojoType='dijit.form.DateTextBox'   datePattern='MM/dd/yyyy' trim='true'  onChange='dojo.byId(\"calendarDiv"+count+"\").innertHTML=arguments[0];' invalidMessage='<bean:message key="dbconfig.search.calender.format"  />' /></div>";
                        ...
                        userInput=userInput+"<div id='searchDataBlnDiv"+count+"' style='display:none'><select name='searchDataBln"+count+"' id='searchDataBln"+count+"' dojoType='dijit.form.FilteringSelect' class='selectbox' store='booleanStore' searchAttr='attr'></select></div>";
...
<html:form action="/masterTableViewAction" >
...
var listSize = '<%= listArray.length %>';
    ...
    <input type="hidden" name="searchCriteria"
        value="<%=request.getParameter("searchCriteria")!=null? request.getParameter("searchCriteria"):""%>" />
                           ...
                           for (String searchField : searchFields) {
                             ...
                             %>
                                <tr id="searchRow<%=count%>">
                                    <td><select name="field<%=count%>" id="field<%=count%>"
                                    ...
                                    <td>
                                      ...
                                      <div id="searchDataDiv<%=count%>" style="display:<%=(dataType.equals("string") || dataType.equals("number"))?"''":"'none'"%>">
                                        ...
                                        <div id="searchDataDateDiv<%=count%>" style="display:<%=dataType.equals("date")?"''":"'none'" %>" >
                                          <input id="searchDataDate<%=count%>" type="text" style="width:194px;height:1.7em;" dojoType="dijit.form.DateTextBox" datePattern="MM/dd/yyyy" trim="true"  onChange="dojo.byId('calendarDiv<%=count%>').innertHTML=arguments[0];"   invalidMessage="<bean:message key="dbconfig.search.calender.format"  />" />
                                        </div>
                                        ...
                                        <div id="searchDataBlnDiv<%=count%>" style="display:<%=dataType.equals("boolean")?"''":"'none'" %>" >
                                                <select name="searchDataBln<%=count%>" id="searchDataBln<%=count%>"
                                                dojoType="dijit.form.FilteringSelect" class="selectbox"
                                ...
                                <script>
                                 ...
                                 searchOptions.push(new searchOption('<%=fieldName%>','<%=dataType%>','<%=operator%>','<%=searchData%>'));
                                 //dijit.byId('searchData<%=count%>').setValue('<%=searchData%>');
                             </script>
                                <%
                           count++;
                           } // end of if - true part
                           %>
                             ...
                             %>
                                <tr id="searchRow1">
                                    ...
                                    <td><select name="operator1" id="operator1"
                                        dojoType="dijit.form.FilteringSelect" class="selectbox"
                                    ...
                                    <td>
                                      ...
                                      <div id="searchDataDiv1"  >
                                        ...
                                        <div id="searchDataDateDiv1" style="display:'none'">
                                          <input id="searchDataDate1" type="text" style="width:194px;height:1.7em;"  dojoType="dijit.form.DateTextBox" trim="true" datePattern="MM/dd/yyyy" onChange="dojo.byId('calendarDiv1').innertHTML=arguments[0];"   invalidMessage="<bean:message key="dbconfig.search.calender.format"  />" />
                                        </div>
                                        ...
                                        <div id="searchDataBlnDiv1"  style="display:'none'">
                                                <select name="searchDataBln1" id="searchDataBln1"
                                                dojoType="dijit.form.FilteringSelect" class="selectbox"
                                                store="booleanStore" searchAttr="attr"></select>
                                        </div>
    ...
    <table width="101.2%" cellspacing="0" cellpadding="0" align="center">
      ...
      <td height="30" valign="top" class="redtitle" align="right"><img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_all_on.png" id="start_on" style="display:none;cursor:pointer" onClick="gotoPage(1);"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_all_off.png" id="start_off" style="display:none;cursor:pointer"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_one_on.png" id="previous_on" style="display:none;cursor:pointer" onClick="previousPage();"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_one_off.png" id="previous_off" style="display:none;cursor:pointer"/>
        <span style="valign=top"><input type="text" id="enteredPageNo" size="5" name="enteredPageNo" style="height:30;width:50"
                    ...
                    value="<bean:write name="masterTableViewForm" property="pageNo"/> of <bean:write name="masterTableViewForm" property="totalNoOfPages"/>"  disabled="true"/></span> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_one_on.png" id="next_on" style="display:none;cursor:pointer" onClick="nextPage()"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_one_off.png" id="next_off" style="display:none;cursor:pointer"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_all_on.png" id="end_on" style="display:none;cursor:pointer" onClick="gotoPage('<bean:write name="masterTableViewForm" property="totalNoOfPages"/>')" /> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_all_off.png" id="end_off" style="display:none;cursor:pointer"/></td>
    </tr>
                             <!--  <tr>
                                <td   colspan='<bean:write name="masterTableViewForm" property="size"/>'  >   
                                <table width="100%" border=0>
                                    <tr>
                                        <td colspan="2"align="right" width="100%"><img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_all_on.png" id="start_on" style="display:none;cursor:pointer" onClick="gotoPage(1);">&nbsp;<img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_all_off.png" id="start_off" style="display:none;cursor:pointer"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_one_on.png" id="previous_on" style="display:none;cursor:pointer" onClick="previousPage();"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_remove_one_off.png" id="previous_off" style="display:none;cursor:pointer"/>

                                        <td valign="bottom" style="padding-top:0px">
                                        <input type="text" id="pageNosss" name="enteredPageNo"  disabled="true" dojoType="dijit.form.ValidationTextBox"  style="width:50px;height:30px;"
                                                    onkeypress="onEnterGoToPage();"  value="<bean:write name="masterTableViewForm" property="pageNo"/> of <bean:write name="masterTableViewForm" property="totalNoOfPages"/> " />&nbsp;
                                
                                        </td>
                                        <td colspan="2" align="right" ><img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_one_on.png" id="next_on" style="display:none;cursor:pointer" onClick="nextPage()"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_one_off.png" id="next_off" style="display:none;cursor:pointer"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_all_on.png" id="end_on" style="display:none;cursor:pointer" onClick="gotoPage('<bean:write name="masterTableViewForm" property="totalNoOfPages"/>')" /> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_all_off.png" id="end_off" style="display:none;cursor:pointer"/></td>
                                    </tr>
                                </table>
                            ...
                            <!--    
                                <button dojoType="dijit.form.Button"  id="btnFirstPage" name="btnFirstPage" type="button" onclick="gotoPage(1)"> &lt;&lt; </button>
                                <button dojoType="dijit.form.Button"  id="btnPreviousPage" name="btnPreviousPage" type="button" onclick="previousPage()"> &lt; </button>
                                -->
                                
                        <!--        
                                <button dojoType="dijit.form.Button"  id="btnNextPage" name="btnNextPage" type="button" onclick="nextPage()"> &gt;</button>
                                <button dojoType="dijit.form.Button"  id="btnLastPage" name="btnLastPage" type="button" onclick="gotoPage('<bean:write name="masterTableViewForm" property="totalNoOfPages"/>');"> &gt;&gt; </button> &nbsp;
                            ...
                            %>
                              ...
                              <tr  class="bgcolor" onMouseOver="hover(this);" onMouseOut="unHover(this, 1);" onDblClick="openInfoframe(<%=outIndex-1%>, this, '<%=(innnerList.get(0))%>')">
                                    ...
                                    value= "<%=\"\" + (innnerList.get(0))%>" onclick=""/></TD>
                            ...
                            <html:hidden name="masterTableViewForm" property="totalNoOfPages"/>
                            <html:hidden name="masterTableViewForm" property="pageNo"/>
...
<tr class="barColor" align="right" >
                <td colspan='<bean:write name="masterTableViewForm" property="size"/>' align = "right" width="1000">
                    <%if(request.getParameter("fromHeader")==null || request.getParameter("fromHeader").toString().equalsIgnoreCase("null")){ %>
                    ...
                    <button dojoType="dijit.form.Button"  id="idDelete" name="btnDelete" type="button" onClick="return submitForm(this);"> <bean:message key="dbconfig.delete" /> </button>
                </td>
</tr>
</table>
 <script>
