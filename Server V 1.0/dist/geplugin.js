 ...
 * Constants defined.
 */
//alert('geplugin.js loaded');
var jsonObjectt = {"Profile" : [{"User Details": {"Mr":"Kaleem Mohammed","Title":"Web Developer","Organization":"WFP-AE","Department":"OSTF"},
									"E-mail"  : {"Office":"kaleem.mohammed@wfp.org","Private":"kaleem6110@gmail.com"}
							  }],

							  "Contact"  :[ {
								"Telephone Numbers" : { "Mobile(Personal)":"+971 55 360 9384", "Mobile (Mission):" :"+91 9951 205526",
																								"WFP Foodsat":"+1349 2812", "Office" :"+971 4549560 x2811"},
								"Instant Messaging" : { "Skype" :"kmohammed7861","MS Lync":"kaleem.mohammed@wfp.org","gTalk":"kaleem6110@gmail.com",																				"bbPIN":"66AB66CD" },
								"Radio" : { "VHF Callsign" : "Delta Foxtrot 8.8", "VHF WFP Foodsat":"+1349 9085" }
							 }],
				"Tracking" : [{		 "This Device" :{"Device Id" : "trackMe-9772","Device type":"Smartphone"},
									 "Last Known Position" : { "Timestamp(GMT)" : "2013-05-08 09:06:11","Latitude" :"24.89973687","Longitude" :"55.07378787" },
									 "Other Tracked Devices" : { "Digital Radio" : "trackMe-13499085" }
							 }]
...
var ge = null;
    ...
    // add a navigation control
          ...
          updateOptions("Scale Legend", false);
    ...
    function _refreshLinkWithParameters(linkName, params) {
	...
	function openLayerDashboard(layerId, layerName, dashboardName, params) {
		parent.openLayerDashboard(layerId, layerName, dashboardName, params);
	}
    ...
    * Kaleem*/
	...
	function toggleTabs(k)
	{
		//alert('toggleTabs is called :'+k +': total_tabs:'+total_tabs );
		for(var i=1;i<=total_tabs;i++)
		{
			if(i!=k ){ jQuery('#tab'+i).css("display","none");jQuery('li#tabItem_'+i).removeClass("selcted"); }
			else { jQuery('#tab'+i).css("display","block");jQuery('li#tabItem_'+i).addClass("selcted"); }
		}
	}
	var total_tabs=0;
function prepareTemplate( jsonObject)
{
		var rootElementt= "<div id=\"tab-container-1\" class=\"demolayout\" >";
		var tabHtml ="<ul id=\"tab-container-1-nav\" class=\"demolayout\" >";
		var divHTML="";
		var cssClass="";
		total_tabs=0;
		
        jQuery.each(jsonObject, function( tabKey, valueArray )
			 {
					total_tabs = total_tabs+1;		
					
					tabHtml+= "<li class=\"a\" id=\"tabItem_"+total_tabs+"\" onclick=\"toggleTabs("+total_tabs+")\">"+tabKey+"</li>";						
					  jQuery.each(valueArray, function( index, atrrObj )
					  {
						  if(total_tabs>1) cssClass="style=\"display:none;\"";
						  divHTML+= "<div class=\"tab\" id=\"tab"+total_tabs+"\" "+cssClass+" >";
						  jQuery.each( atrrObj, function( attrName, attrValue )
						  {
							  divHTML+= "<fieldset><legend>"+attrName+"</legend>";
							  jQuery.each( attrValue, function(colname, colvalue)
							   {
									 divHTML+= "<div class=\"blockrow\"><label>"+colname+" </label>"+	 colvalue+"</div>";
							   });

						   });
						 divHTML+="</fieldset></div>";	
					  });			 

			 });
			tabHtml+="</ul>";	
			 rootElementt+= tabHtml +divHTML+"</div>";

			return rootElementt;
}
var photoSrc = "";
    function openFlashBalloon(currentPlaceMark) {
		//alert('openFlashBalloon : '+currentPlaceMark.getDescription() );
  ...
  // alert(  currentPlaceMark.getDescription() );
  if( description.indexOf("DELIM") > 0 )
		{
			var finalHTML='';

			  var s=  new String( currentPlaceMark.getDescription() );
			  var from = s.indexOf("DELIM");
			  var to = s.lastIndexOf("DELIM");
			   s = s.substring(from+5, to);
			 // alert(s);
			  s = s.replaceAll("&#123;","{");
			  s = s.replaceAll("&#125;","}");
			  s = s.replaceAll("&#091;","[");
			  ...
			  s = s.replaceAll("&quot;","\"");
				//alert(s); 
			var result = prepareTemplate ( jQuery.parseJSON( s ) );

			if( description.indexOf(":jpegPhoto:")>0 )
				{	//alert('inside');
						var iFrom =  description.indexOf(":jpegPhoto:"); var imgLocation='../../images/user_male.png';
						var iTo =  description.lastIndexOf(":jpegPhoto:"); if( description.indexOf('Miss') >0 ) imgLocation='../../images/user_female.jpg';
						if( description.substring(iFrom+11, iTo) != 'null' )
						finalHTML =  '<div style="background-image:url(data:image/jpeg;base64,'+ description.substring(iFrom+11, iTo)+');background-position:right top;margin-right:10px;background-repeat:no-repeat;background-size: auto 100px;" >';
						else finalHTML =  '<div style="background-image:url('+imgLocation+');background-position:right top;margin-right:10px;background-repeat:no-repeat;background-size:100px 80px;" >';
						finalHTML+=' <b><font size="2" face="Verdana">'+currentPlaceMark.getName()+'</font></b><br/>'+result;
						finalHTML+='</div>';
				
				} else 	finalHTML+=' <b><font size="2" face="Verdana">'+currentPlaceMark.getName()+'</font></b><br/>'+result;
				//alert(  finalHTML );

			 div.innerHTML = finalHTML;					
			 b.setContentDiv(div);
			 ge.setBalloon(b);
			 //setTimeout(toggleTabs(1), 3000);
		}
		else
		{
	...
	function extractEcosessionEvents(xmldoc, nLink){
		try{
			...
			if(extendedData != null && extendedData.length > 0){
				var dataArray = extendedData[0].getElementsByTagName(KML_DATA);
				if(dataArray != null){
