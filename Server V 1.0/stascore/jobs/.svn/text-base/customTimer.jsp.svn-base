...
com.enterprisehorizons.magma.jobs.forms.CustomTimerForm,com.spacetimeinsight.db.scheduler.model.ServerEnvMaster,com.spacetimeinsight.db.model.util.DataModelsCache, com.spacetimeinsight.db.scheduler.model.*"%>
         ...
         function submitForm(btn) {
            ...
            if(btn.value == dijit.byId('idBack').value){
                ...
                if(!isValidForSubmit)
                    return false;
              /*
                isParamsValid = validateUniqueInParamsEcosystem(trimAll(dijit.byId('timerParameterValueMap').attr('value')));
                if(!isParamsValid)
                    return false;
                    
                isParamsValid = isValidParameters(trimAll(dijit.byId('timerParameterValueMap').attr('value')),'true');
...
<html:form action="customTimerAction.do?operation=onUpdateCustomTimer" method="get">
				...
				<tr>
					...
					<td style="padding-left:10px;align:left;" width="90px">
						<input type="text"  id="jobName" name="jobName"  class="medium1" style="height:1.7em;" maxlength="45"
							dojoType="dijit.form.ValidationTextBox" required="true" trim="true" ucfirst="true"  value="<bean:write name="customTimerForm" property="jobName" />" />                             
					</td>
				</tr>     
				<tr>                                       
		...
		<tr>
			...
			<table cellpadding="10" cellspacing="" border="0" width="100%">
				<tr>
					...
					<td style="padding-left:10px;align:left;" width="90px">
						<textarea dojoType="dijit.form.SimpleTextarea" rows="5" cols="30" id="description" class="medium1" style="height:5.7em;" name="description"  ><bean:write name="customTimerForm" property="description" /></textarea>
					</td>
				</tr>     
				<tr>                                       
					<td style="width:90px;" >&nbsp;</td>
...
<tr>
	...
	<table cellpadding="10" cellspacing="" border="0" width="300" >
				<tr>
					...
					<td height="27" align="left"  colspan="2" >
						<select  id="jobType" dojoType="dijit.form.FilteringSelect"name="jobType" autoComplete="false"onchange="javascript:statusChange();" value="<bean:write name="customTimerForm" property="jobType" />">
       ...
       <div id="div5" style="display:block;" >
            <table class="tableBgColor">
                ...
                <input type="text"  id="timerTask" name="timerTask" style="height:1.7em;"
                                            dojoType="dijit.form.ValidationTextBox" required="true" trim="true" ucfirst="true" value="<bean:write name="customTimerForm" property="timerTask" />"    />                                                         </td>
                                                        </tr>       
                                                    </table>
                                            </td> 
                                           <td style="width:40px;">&nbsp;
                                    ...
                                    <textarea dojoType="dijit.form.SimpleTextarea" rows="5" cols="30" id="timerParameterValueMap"  style="width:210px;height:80px;" rows="6"
                                    name="timerParameterValueMap"  trim="true" ><bean:write name="customTimerForm" property="timerParameterValueMap" /></textarea>
...
<table border="0" width="101.2%">
     <tr class="barColor">
          <td colspan="7"  align="center" style="padding-left:335px;"> 
            
 <input type="hidden" name="operation" value="onUpdateCustomTimer">
...
function loadFormValues(){
	...
	if(dijit.byId('startTimeType').value == START_JOB_AFTER_SERVER_START){
		...
		populateAbsoluteStartTypeValues('<bean:write name="customTimerForm" property="cronTypeId" />', status);
			...
			dojo.byId('expression').value = '<bean:write name="customTimerForm" property="expression" />';
