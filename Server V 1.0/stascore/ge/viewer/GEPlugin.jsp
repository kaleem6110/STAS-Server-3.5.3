...
%>
<script src="<%=(ServerUtils.isHttpsEnabled())?"https":"http"%>://www.google.com/jsapi?key=<%=geKey%>"></script>
<%
} else {
String[] strArray = StringUtils.split(geeServerUrl, "/");
String earthServerRootUrl = strArray[0]+"//"+strArray[1]+"/";
%>
<script type="text/javascript" src="<%=earthServerRootUrl%>/js/earth_plugin_loader.js"></script>
<%
}
%>
...
<script>
 ...
 <%
  ...
  %>
    ...
    if(locale != 'en-US') {
        google.load("maps", "3.8", {"language":locale,"other_params":"sensor=false"});
    } else {
  ...
  %>
    google.load("earth", "1", {"other_params":"client=<%=geClientId%>&sensor=false"});
    google.load("maps", "3.8", {"other_params":"client=<%=geClientId%>&sensor=false"});
  <%
    }
  %>
...
function _highlight(coordStr,dashboardId,layerId) {
      ...
      var hKey = layerId+"_"+dashboardId;
    	 removeAllChildren(hKey);
     ...
     // if(placemark == null) {
	    ...
	    highlightPlacemarksArray[hKey] = placemarksArray;
 }
    ...
    function geDistanceClickListener(kmlEvent){
        try{
            ...
            if(distance.toFixed(2) == 0.00)
            {
            	...
            	endPlacemark.setName((distance * MILE_TO_FEET ).toFixed(2) + FEET_SYMBOL);
            }
            else
            {
            	//distance in mile
            	endPlacemark.setName(distance.toFixed(2) + MILE_SYMBOL);
            }
...
*/
    ...
    function getDistance(startPlacemark , endPlacemark ){
        if(startPlacemark != null && endPlacemark != null){
            ...
            return (new geo.Point(startPlacemark.getGeometry()).distance(new geo.Point(endPlacemark.getGeometry()))/ 1609.344).toFixed(5);
        }
    }
...
//********* Smart Art code   *********//
    ...
    */
    ...
    function smartArtHighlight(mouseEvent){
        var magmaVSUrl = "<%=serverUrl %>noContentServlet/PNGStreamer";
