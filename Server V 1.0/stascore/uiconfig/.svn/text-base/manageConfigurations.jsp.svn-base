...
//          showEmptyDialog('Please click (+) to add tab ', "Manage Configurations");          
 ...
 {
      if(!focusHome && (tmp.toUpperCase() == ADD_SECTION.toUpperCase() || tmp.length == 0)) 
     {
     ...
     if(tmp.toUpperCase() != ADD_SECTION.toUpperCase() && tmp.length != 0)
     	newXmlName = tmp;
...
%>
  <button dojoType="dijit.form.Button"  type="button" id="idHome" name="btnHome" 
  onclick="javascript:submitToHome();" onmouseover="setFocusHome(true);" onmouseout="setFocusHome(false);" onfocus="setFocusHome(true);" onblur="setFocusHome(false);" >
      <bean:message key="uiconfig.home" bundle="uiconfig"/> 
  </button>
<%}%>
