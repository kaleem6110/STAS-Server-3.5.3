...
<script type="text/JavaScript">
    ...
    function submitForm(btn) {
        ...
        if(id == "Add") {
			...
			frm.action = "<%=ServerUtils.getContextName(request)%>/admin/shp2dbmappingaddselect.jsp";
        } else {
            if(dijit.byId('cmbMappedDefinitions') != null && frm.cmbMappedDefinitions.selectedIndex == 0) {
                ...
                if(dijit.byId('cmbMappedDefinitions') != null)
				 ...
				 if(dijit.byId('cmbMappedDefinitions') != null && dijit.byId('cmbMappedDefinitions').value != ''){
                    frm.action = "<%=ServerUtils.getContextName(request)%>/admin/shp2dbmappingview.jsp";
					 frm.submit();
			       }
                   else{
					...
					if(dijit.byId('cmbMappedDefinitions') != null && dijit.byId('cmbMappedDefinitions').value != ''){
						if(!validateShapeFiles()){
						 ...
						 frm.action = "<%=ServerUtils.getContextName(request)%>/admin/shp2dbmappingupdate.jsp";
						  frm.submit();
					}
					else{
                         showEmptyDialog("<bean:message key='shpDbMap.check.correct.value' bundle='shpDbMap'/>","<bean:message key='admin.common.alert.title' bundle='admin'/>");
...
<form name="frm" method="post" action="<%=ServerUtils.getContextName(request)%>/admin/shp2dbmappingaddselect.jsp">
        ...
        %>
			...
			<tr>
	            ...
	            <td style = "padding-left: 10px;padding-top: 20px;">  
			        ...
			        <select id = "cmbMappedDefinitions" name="cmbMappedDefinitions" dojoType="dijit.form.FilteringSelect">
					...
					%>  
						<option value="<%=mappedFiles[i]%>"><%=mappedFiles[i]%></option>
						<%	if(!Shp2DBMappingUtils.isSupportedShapeFilesPresent(mappedFiles[i])){
								%>	
									<script>
										invalidShapeFileArray[<%=i%>] = '<%=mappedFiles[i] %>';
									</script>	
						<%		
							}
						%>	
